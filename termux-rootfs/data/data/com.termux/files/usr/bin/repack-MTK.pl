...
sub repack_boot {
    ...
    die_msg("cannot find 'mkbootimg' tool!")
        if system("command -v mkbootimg >/dev/null 2>&1");

	if ($debug_mode) {
		print colored ("\nBuild $type image command:", 'yellow') . "\n";
		print "'mkbootimg --kernel $kernel --ramdisk temp-$ramdiskfile @extrargs -o $outfile'\n";
	}
	system ("mkbootimg --kernel $kernel --ramdisk temp-$ramdiskfile @extrargs -o $outfile");
...
sub repack_logo {
	...
	for my $inputfile (glob "./$filename-img[*.*") {
		...
		if ($extension eq ".png") {
			die_msg("couldn't find 'GraphicsMagick' tool! Please install it and try again.")
				if system ("command -v gm >/dev/null 2>&1");
...
sub png_to_rgb565 {
	...
	system ("gm convert -depth 8 $filename.png rgb:$filename.raw");
