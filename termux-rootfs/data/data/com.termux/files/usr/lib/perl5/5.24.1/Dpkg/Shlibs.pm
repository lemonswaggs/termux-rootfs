...
use constant DEFAULT_LIBRARY_PATH =>
    qw(/data/data/com.termux/files/usr/lib);
...
sub setup_library_paths {
    ...
    if ($multiarch) {
        push @system_librarypaths, "/data/data/com.termux/files/usr/lib/$multiarch";
    }

    push @system_librarypaths, DEFAULT_LIBRARY_PATH;

    # Update library paths with ld.so config.
    parse_ldso_conf('/data/data/com.termux/files/usr/etc/ld.so.conf') if -e '/data/data/com.termux/files/usr/etc/ld.so.conf';
